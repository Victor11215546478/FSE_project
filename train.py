"""
Code for training Deep Kd-Networks
"""
...
# loop of training
for it in range(101): # 10000
    optimizer.zero_grad()
    losses = []
    corrects = []
    for batch in range(10):
        j = np.random.randint(len(d))
        ...
        cutdim_v = [
            (torch.from_numpy(np.array(item).astype(np.int64))) for item in cutdim
        ]
    ...
    optimizer.step()
    print(
        f"batch: {it}, loss: {np.mean(losses)}, correct {np.sum(corrects)}"
    )

    if (it % 50 == 0) & (it > 0): # 1000
        torch.save(NET.state_dict(), f"save_model_{it}.pth")
